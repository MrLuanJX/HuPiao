...
+ (void)showWithImages:(NSArray *)imgs titles:(NSArray *)titles selectBlock:(void (^)(NSInteger))selectBlock view :(UIView *)view{
    LLWPlusPopView *bg = [[LLWPlusPopView alloc] initWithFrame:CGRectMake(0, 0, HPScreenW, HPScreenH)];//  - k_bar_height
...
- (void)setupMainView {
    ...
    pageControl.pageIndicatorTintColor = HPUIColorWithRGB(0xc0c0c0, 1.0);//HexColorInt32_t(c0c0c0);
    pageControl.currentPageIndicatorTintColor = HPUIColorWithRGB(0x1fb497, 1.0);//HexColorInt32_t(1fb497);
...
- (void)setupItem {
    ...
    for (NSInteger i = 0; i < self.imageNames.count; i ++) {
        ...
        [button setTitleColor:HPUIColorWithRGB(0x666666, 1.0) forState:UIControlStateNormal];
