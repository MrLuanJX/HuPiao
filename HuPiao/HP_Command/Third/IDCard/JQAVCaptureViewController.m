...
//    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageWithColor:[UIColor clearColor] size:CGSizeMake(BMScreenW, StatusBarHeight+self.navigationController.navigationBar.height)] forBarMetrics:UIBarMetricsDefault];
...
//    [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:UIColorWithRGB(0x323232)}];
    
    // 将AVCaptureViewController的navigationBar调为不透明
//    [[[self.navigationController.navigationBar subviews] objectAtIndex:0] setAlpha:1];
    [self stopSession];
//    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor] size:CGSizeMake(BMScreenW, StatusBarHeight+self.navigationController.navigationBar.height)] forBarMetrics:UIBarMetricsDefault];
...
-(void)showAuthorizationDenied {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // 更新界面
        [AlertViewTool showAlertWithTitle:@"提示" message:@"请在“设置-闪电借款-相机”选项中，允许访问您的相机" clickAtIndex:^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex==1) {
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    if ([self.outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange]] || [self.outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange]]) {
        ...
        if ([captureOutput isEqual:self.videoDataOutput]) {
            // 身份证信息识别
            [self IDCardFontRecognit:imageBuffer];
        }
    } else {
        NSLog(@"输出格式不支持");
    }
...
#pragma mark - 身份证信息识别
- (void)IDCardFontRecognit:(CVImageBufferRef)imageBuffer {
...
//            iDInfo.type = ctype;            
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.CaptureInfo) {
                        self.CaptureInfo(iDInfo,image,subImage);
                    }
                    [self.navigationController popViewControllerAnimated:YES];
                });
            }
